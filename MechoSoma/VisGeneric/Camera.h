...
{
private: // первичные значени€
	cMatrix				Matrix;
	Vect3f				Pos;						// местоположение камеры
	Vect3f				PosOfs;						// смещение центра при наклоне
	Vect3f				Angle;						// углы Ёйлера определ€ющие матрицу, в градусах
	...
	Vect2f				Focus;						// Focus.z - фокус, пор€дка единицы
	Vect2f				zPlane;						// рассто€ние до ближайшей и дальней плоскостей обрезани€, используетс€ дл€ zBuffer'а
	Vect2f				zBufferScale;				// zBufferScale.x=zNear,zBufferScale.y=zFar - 0..1 коэффициенты масштабировани€ zBuffer'а
	cRenderDevice		*RenderDevice;				// устройство вывода
	cBaseArray <sPlane4f>PlaneClip3d;				// плоскости отсечени€
public:
	cCamera();
	~cCamera();
	...
	// функции позиционировани€ камеры и дл€ изменени€ ее матрицы
	...
	// функции дл€ работы с видимой областью
	...
	// функции дл€ работы с пирамидой видимости
	...
	// функции конвертации, матрица и координаты должны быть зациклены и нат€нуты на шар
	inline void BuildMatrix(MatXf &matrix,const Vect3f &pos);		// создает матрицу matrix, на основе cCamera::Matrix && pos, зацикливает и нат€гивает на шар
	inline void LoadMatrix(Vect3f &pos);	// устанавливаетс€ pos - зацикливаетс€ и нат€гиваетс€ на шар
	...
	// функции теста видимости, координаты должны быть зациклены и нат€нуты на шар (полна€ матрица визуализации)
	eConstVisible TestVisible(const MatXf &matrix,const Vect3f &min,const Vect3f &max);	// дл€ BoundBox с границами min && max
	eConstVisible TestVisible(const MatXf &matrix,const Vect3f &size);		// дл€ параллелепипеда размера size, левый нижний угол лежит в matrix
	eConstVisible TestVisible(const MatXf &matrix,float radius);			// дл€ плоского круга параллельного экрану
...
extern Vect2f GlobalWorldScale;	// GlobalWorldScale.x,GlobalWorldScale.y - масштабный коэффициент дл€ зацикливани€ = 1.f
