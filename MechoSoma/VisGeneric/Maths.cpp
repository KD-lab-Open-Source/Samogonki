...
void InitArray()
{
	if(CosNChar==0) // òàáëèöà cos â ñòåïåíè COS_DEGREE òèïà char äëÿ èñòî÷íèêà ñâåòà cLight
	{	
		CosNChar=new unsigned char [128+3]; // 3 - ýòî èç-çà íåòî÷íîñòè â ðàñ÷åòàõ
		int j;
		for(j=0;j<(128+3);j++)
		{
			int i=j;
	...
	if(OmniLightMap==0) // ¤¢ã¬¥à­ ï äã­ªæ¨ï ã¡ë¢ ­¨ï ¨­â¥­á¨¢­®áâ¨ â®ç¥ç­®£® ¨áâ®ç­¨ª  cOmni
	{
