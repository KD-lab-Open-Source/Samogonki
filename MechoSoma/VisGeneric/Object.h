...
class cLight
{
	Vect3f		Pos;
	Vect3f		Angle;			
	Vect3f		Tangent;		// вектор направлени€ источника освещени€
	sColor4f	Ambient;		// фонова€ составл€юща€
	sColor4f	Diffuse;		// диффузна€ составл€юща€
	cMatrix		Matrix;
public:
	cLight();	
	~cLight();
	...
	__forceinline int CalcColorDM(int xNormal,int yNormal,int zNormal,
						 int xvLight,int yvLight,int zvLight,
						 int xvView, int yvView, int zvView,int cosLightView);
	// расчет €ркости источника диффузной
