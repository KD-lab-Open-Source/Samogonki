...
public:
	...
	cBaseLink()												{ next=prev=0; }
	~cBaseLink()											{ Detach(); }
	void Attach(cBase *Link)
	{
		cBase *Next=next; Link->next=Next;
		Link->prev=(cBase*)this; next=Link;
		if(Next) Next->prev=Link;
	}
	void AttachEnd(cBase *Link)
	{
		cBase *tmp=this;
...
public:
	...
	inline cBase* FindID(unsigned int ID)						{ for(cBaseList *start=BaseList; start; start=start->next) if(start->Base->ID==ID) return start->Base; return 0; }
};
