...
//-----------------------------------------------------------------------------
static LPDIRECTDRAW7 g_pDD                = NULL;  // Used for callbacks
											 ...
											 HMONITOR hMonitor )
{
	...
	{
        DEBUG_MSG( TEXT("Can't query IDirectDraw7 during enumeration!") );
		pDD->Release();
		return D3DENUMRET_OK;
	}
	pDD->Release();
...
//-----------------------------------------------------------------------------
HRESULT D3DEnum_SelectDefaultDriver( DWORD dwFlags, DWORD dwWidth, DWORD dwHeight, DWORD dwBpp, HRESULT (*AppConfirmFn)(DDCAPS*, D3DDEVICEDESC*) )
{
			 ...
			 pDriver = pDriver->pNext )
		{
				 ...
				 pDevice = pDevice->pNext )
			{
				...
				if( bFound )
				{
					...
					if( AppConfirmFn )
						if( FAILED( AppConfirmFn( &pDriver->ddDriverCaps, &pDevice->ddDesc ) ) )
							bFound = FALSE;
				}

				unsigned int dwSupportBpp=16;
				for( D3DEnum_ModeInfo* p = pDevice->pFirstMode; p; p=p->pNext ) 
					if(p->ddsd.ddpfPixelFormat.dwRGBBitCount==32)
						dwSupportBpp=32;
				if(dwSupportBpp>dwBpp) dwSupportBpp=dwBpp;

				if( bFound )
				{
					for( D3DEnum_ModeInfo* pMode = pDevice->pFirstMode; pMode; pMode = pMode->pNext ) {
						if( (dwWidth == pMode->ddsd.dwWidth && 
							dwHeight == pMode->ddsd.dwHeight &&
							dwSupportBpp == pMode->ddsd.ddpfPixelFormat.dwRGBBitCount)
							|| dwWidth==0) {
							g_pCurrentDriver = pDriver;
							g_pCurrentDriver->pCurrentDevice = pDevice;
							g_pCurrentDriver->pCurrentDevice->pCurrentMode = pMode;
							return S_OK;
						}
					}
				}
