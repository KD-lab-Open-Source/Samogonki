...
//-----------------------------------------------------------------------------
VOID D3DUtil_InitViewport( D3DVIEWPORT2& vp, DWORD dwWidth, DWORD dwHeight, RECT *rcClipRect )
{
    ZeroMemory( &vp, sizeof(D3DVIEWPORT2) );
    vp.dwSize   = sizeof(D3DVIEWPORT2);
    vp.dwX = rcClipRect->left;
    vp.dwY = rcClipRect->top;
    vp.dwWidth  = rcClipRect->right - rcClipRect->left;
    vp.dwHeight = rcClipRect->bottom - rcClipRect->top;

    vp.dvClipX = 0;
    vp.dvClipY = 0;
    vp.dvClipWidth = (float)dwWidth;
    vp.dvClipHeight = (float)dwHeight;
...
{
    MD3DLOG2( 1, "%s(%ld): ", strFile, dwLine );
    MD3DLOG( 1, strMsg );
