...
static int aaa = 0;
struct Map : public hash_map<int, int>
{
	Map() : hash_map<int, int>(1000) {}
	void call(int addr)
	{
		iterator i;
		if(!empty() && (i = find(addr)) != end()){
			if(i -> second++ > 500000)
				aaa = 1/aaa;
			}
		else
			insert(value_type(addr, 1));
	}
};

Map map;

void addr_handler(int code)
{
        static char buf[16];
        int* pi = &code;pi--;
	map.call(*pi);
...
{
	XStream f("f", XS_OUT);
	typedef multimap<int, int> Smap;
	Smap smap;
	Map::iterator i;
	FOR_EACH(map, i)
		smap.insert(Smap::value_type(i -> second, i -> first));
	int n = 0;
	{
	Smap::iterator i;
	FOR_EACH(smap, i){
