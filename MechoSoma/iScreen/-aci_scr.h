...
struct aciScreenResourceHBM: aciScreenResource {
	void load();
	void free();
	unsigned getColor(int x,int y,int frame);
...
struct aciScreenKeyObject
{
	...
	void AddKey(int code,int optFlags);
...
struct aciScreenEvent : XListElement
{
	...
	void AddKey(int code, int optFlag){ keyObj -> AddKey(code, optFlag); }
...
//@caiiiycuk: missed identifiers
#define ACS_HIDDEN  0x20
#define ACS_BLOCKED 0x40
...
struct aciScreenInputField : aciScreenObject
{
	...
	int Color1;
	int Color2;
	int AlphaCur, AlphaDest, Alpha0, Alpha1, Alpha2;
...
struct aciScreenScroller : aciScreenObject
{
	...
	int Color;
	int BackColor;
	int AlphaCur, AlphaDest, dAlpha, Alpha0, Alpha1, Alpha2;
