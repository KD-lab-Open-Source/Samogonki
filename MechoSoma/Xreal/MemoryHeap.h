/*/////////////////////////////////////////////////////////////////////////////////////////////////////
	ћногоблочна€ куча п€м€ти
 
  Using:
	
	MemoryHeap mh( [начальный размер], [максимальное кол-во блоков], [название - дл€ вывода статистики] );

	mh.init(); - освобождает все блоки кроме первого, в котором обнул€ет смещение

	p = mh.malloc(size);
	p = mh.calloc(number, size_of_type);
	p = mh.realloc(p, old_size); - оптимален только дл€ последнего блока

	mh.store();  - запоминание дл€ неполной инициализации кучи, вложенность неограничена
	mh.restore(); - неполный откат кучи.

	1.  аждый новый блок в два раза больше предыдущего.
	2. »спользование статистики (_MEMORY_HEAP_STATISTICS_) позвол€ет выбрать оптимальный начальный размер.
	3. ѕри превышении максимального количества блоков генерируетс€: trow "MemoryHeap - out of memory";
	4. ƒл€ отладки можно перейти к обычному выделению: _MEMORY_HEAP_STD_MALLOC_
...
public:
	...
	void init(); // ќсвобождает все дополнительные блоки и сбрасывает указатель текущего блока
	void store(); // ”станавливает метку дл€ неполной очистки (модель стека)
	void restore();  // Ќеполна€ очистка по установленной ранее метке

	int allocated() const { return allocated_memory + size; } // ¬ыделенна€ пам€ть
	int used() const { return used_memory + offset; } // –еально используема€
