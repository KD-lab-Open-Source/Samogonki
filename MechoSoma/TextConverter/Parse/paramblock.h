...
public:
	...
	inline void* GetValue(){char* p;for(p=name;*p++;);return p;};
	inline int GetInt(){ERRORM(type==P_INT,"OneParam::MustInt"); return *(int*)GetValue();};
	inline float GetFloat()
	{
		if(type==P_INT) return (float)GetInt();
		ERRORM(type==P_FLOAT,"OneParam::MustFloat"); return *(float*)GetValue();
	}
	inline char* GetString(){ERRORM(type==P_STRING,"OneParam::MustString");return (char*)GetValue();}
	inline ParamBlock* GetBlock(){ERRORM(type==P_PARAMBLOCK,"OneParam::MustParamBlock");return (ParamBlock*)GetValue();}
	inline P_FArray* GetFArray(){ERRORM(type==P_FARRAY,"OneParam::MustFArray");return (P_FArray*)GetValue();}
