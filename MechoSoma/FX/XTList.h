/******************************************************************
  ...
  ѕри _DEBUG'е на каждую операцию добавлени€/освобождени€ включена проверка

  Ќеобходимые элементы:
  struct Type {
	int ID; // не об€зателен, если не используешь search()
	Type* next;		
	Type* prev;
	Type(){ next = prev = 0; } // необходимо обнул€ть
	};
  ...
  ƒл€ помещени€ одного элемента в несколько списков используйте шаблоны
   XTList1<Type> list1;	
   XTList2<Type> list2;	
  и т.д. (пока, правда, до 2). 
  Ёлемент в этом случае должен содержать проинициализированные
...
public:
		...
		XTList();
		~XTList();   // clear(), т.е. не удал€ет элементы.
	
	int size();		// число элементов
	Type* first();		// первый
	Type* last();		// последний

	void clear();		// удал€ет все элементы только из списка
	void delete_all();   // удал€ет все элементы из списка и пам€ти

	void insert(Type* p);		// добавление в начало списка
	void append(Type* p);		// добавление в конец списка
	void insert(Type* pointer,Type* p);	// добавление перед pointer (используетс€ при сортировке)
...
public:
		...
		XTList1();
		~XTList1();   // clear(), т.е. не удал€ет элементы.
...
public:
		...
		XTList2();
		~XTList2();   // clear(), т.е. не удал€ет элементы.
