...
int s_sur_scr::load_scr(char* fname)
{
	...
	XStream f(0);
#ifdef _MECHOSOMA_
	if(!vmapOpenResource(fname, f)) return 0;
#else
	...
	if(script.end <= script.begin) {delete world; delete buft; f.close(); return 0;} //����� ���� ��� �������
...
void s_sur_scr::load_data()
{
	XStream f(0);
#ifdef _MECHOSOMA_
	if(!vmapOpenResource(name_scr, f)) return;
#else
	...
	while( !f.eof()) { f.read(&b,1); if(b=='}') break; }//����� ����� �������
...
//	char* world=new char[256];
	...
	while( !fi.eof() ) { fi.read(&b,1); fo.write(&b,1); if(b=='}') break; } //����� ����� �������
...
void s_sur_scr::lex2convert(char* world, char*& buf, XStream &fout)
{
	if(strcmp(world,"convert")==0){ // conver AK last double AKM = 11
		...
		if(num_old_row==-1) ErrH.Abort("�� ������������ ��� ���������� ������� �������",XERR_USER,0,world);
		get_lex(buf, world);
		...
		for(i=0; i<numbers_cell; i++){
			...
			if(row[num_old_row].type==INT_VARIABLE){ //���� ������ ������ - Int
			...
			if(row[num_old_row].type==DOUBLE_VARIABLE){
				...
				if(row[numbers_rows].type==CHAR_VARIABLE){
					...
					row[numbers_rows].set_el(i,(char*)bbb);					
				}
			}
...
// ����������� Updata �������
	...
	int ver_up;
	f >=ver_up;
	if(ver_up <= version ) ErrH.Abort("������ ������ Updata �������");
