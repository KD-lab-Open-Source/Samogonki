...
const	int XGR_MASK_R1	= 0x7C00;
const	int XGR_MASK_G1	= 0x03E0;
const	int XGR_MASK_B1	= 0x001F;

const	int XGR_MASK_R0	= 0xF800;
const	int XGR_MASK_G0	= 0x07E0;
const	int XGR_MASK_B0	= 0x001F;

const	int XGR_SHIFT_R1	= 10;
const	int XGR_SHIFT_G1	= 5;
const	int XGR_SHIFT_B1	= 0;

const	int XGR_SHIFT_R0	= 11;
const	int XGR_SHIFT_G0	= 5;
const	int XGR_SHIFT_B0	= 0;

const	int XGR_COLOR_MASK_R1	= 0x001F;
const	int XGR_COLOR_MASK_G1	= 0x001F;
const	int XGR_COLOR_MASK_B1	= 0x001F;

const	int XGR_COLOR_MASK_R0	= 0x001F;
const	int XGR_COLOR_MASK_G0	= 0x003F;
const	int XGR_COLOR_MASK_B0	= 0x001F;
...
typedef int  (*XGR_TextWidthHandler)(void* text,int font,int hspace,int vspace);
typedef int  (*XGR_TextHeightHandler)(void* text,int font,int hspace,int vspace);
...
struct XGR_Screen
{
	...
	void flush625(int x,int y,int sx,int sy);
	...
	int init(int x,int y,int flags,void *hWnd=0);
	void close(void);
	void finit(void);
	...
	void set_entries(int start,int count);
...
struct XGR_Mouse
{
	...
	int MovementX; // В текущей реалиации не сбрасываются, лучше не использовать
	int MovementY;
	int MovementZ;
...
#define XGR_Flush625(x,y,sx,sy)				XGR_Obj.flush625(x,y,sx,sy)
